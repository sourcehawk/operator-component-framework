...
// Mutation defines a mutation that is applied to a statefulset Mutator
...
type volumeClaimTemplateOp struct {
	name string
	pvc  *corev1.PersistentVolumeClaim // nil for remove
}
...
// applied to the StatefulSet in a single controlled pass when Apply() is called.
//
...
// changes to the StatefulSet. The constructor creates the initial feature scope,
...
// EnsureContainer records that a regular container must be present in the StatefulSet.
...
// EnsureInitContainer records that an init container must be present in the StatefulSet.
...
//
// Planning:
// All statefulset spec edits are stored and executed during Apply().
//
// Execution Order:
//   - Within a feature, edits are applied in registration order.
...
func (m *Mutator) EditStatefulSetSpec(edit func(*editors.StatefulSetSpecEditor) error) {
	...
	m.active.statefulsetSpecEdits = append(m.active.statefulsetSpecEdits, edit)
}
...
func (m *Mutator) EditObjectMetadata(edit func(*editors.ObjectMetaEditor) error) {
	...
	m.active.statefulsetMetadataEdits = append(m.active.statefulsetMetadataEdits, edit)
}
...
// in all containers of the StatefulSet.
//
...
// removed from all containers of the StatefulSet.
//
// This is a convenience wrapper over EditContainers.
...
func (m *Mutator) Apply() error {
	for _, plan := range m.plans {
		// 1. Object metadata
		if len(plan.statefulsetMetadataEdits) > 0 {
			editor := editors.NewObjectMetaEditor(&m.current.ObjectMeta)
			for _, edit := range plan.statefulsetMetadataEdits {
		...
		// 2. StatefulSetSpec
		if len(plan.statefulsetSpecEdits) > 0 {
			editor := editors.NewStatefulSetSpecEditor(&m.current.Spec)
			for _, edit := range plan.statefulsetSpecEdits {
				if err := edit(editor); err != nil {
					return err
				}
			}
...
func applyVolumeClaimTemplateOp(vcts *[]corev1.PersistentVolumeClaim, op volumeClaimTemplateOp) {
	found := -1
	for i, v := range *vcts {
		if v.Name == op.name {
			found = i
	...
	if op.pvc == nil {
		// Remove
		if found != -1 {
			*vcts = append((*vcts)[:found], (*vcts)[found+1:]...)
		}
		return
	}
