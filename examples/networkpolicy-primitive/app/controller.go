...
type ExampleController struct {
	...
	// NewNetworkPolicyResource is a factory function to create the networkpolicy resource.
	// This allows us to inject the resource construction logic.
	NewNetworkPolicyResource func(*sharedapp.ExampleApp) (component.Resource, error)
}
...
func (r *ExampleController) Reconcile(ctx context.Context, owner *sharedapp.ExampleApp) error {
	// 1. Build the networkpolicy resource for this owner.
	npResource, err := r.NewNetworkPolicyResource(owner)
	if err != nil {
		return err
	}

	// 2. Build the component that manages the networkpolicy.
	comp, err := component.NewComponentBuilder().
		WithName("example-app").
		WithConditionType("AppReady").
		WithResource(npResource, component.ResourceOptions{}).
		Build()
	if err != nil {
		return err
	}
