...
type FieldApplicationFlavor flavors.FieldApplicationFlavor[*appsv1.ReplicaSet]

// PreserveCurrentLabels ensures that any labels present on the current live
// ReplicaSet but missing from the applied (desired) object are preserved.
// If a label exists in both, the applied value wins.
func PreserveCurrentLabels(applied, current, desired *appsv1.ReplicaSet) error {
	return flavors.PreserveCurrentLabels[*appsv1.ReplicaSet]()(applied, current, desired)
}

// PreserveCurrentAnnotations ensures that any annotations present on the current
// live ReplicaSet but missing from the applied (desired) object are preserved.
// If an annotation exists in both, the applied value wins.
func PreserveCurrentAnnotations(applied, current, desired *appsv1.ReplicaSet) error {
	return flavors.PreserveCurrentAnnotations[*appsv1.ReplicaSet]()(applied, current, desired)
}

// PreserveCurrentPodTemplateLabels ensures that any labels present on the
// current live ReplicaSet's pod template but missing from the applied
// (desired) object's pod template are preserved.
// If a label exists in both, the applied value wins.
func PreserveCurrentPodTemplateLabels(applied, current, _ *appsv1.ReplicaSet) error {
...
// on the current live ReplicaSet's pod template but missing from the applied
// (desired) object's pod template are preserved.
// If an annotation exists in both, the applied value wins.
func PreserveCurrentPodTemplateAnnotations(applied, current, _ *appsv1.ReplicaSet) error {
