...
- **A partly suspended CR reports `True`/`Suspended`, and a gate-disabled component counts as ready.** Both are in their
  expected state. If owner-level suspension must be an explicit state, branch on the owner's own suspend field before
  calling `Aggregate`.
