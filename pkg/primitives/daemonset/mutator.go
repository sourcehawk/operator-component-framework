...
// Mutator is a high-level helper for modifying a Kubernetes DaemonSet.
//
// It uses a "plan-and-apply" pattern: mutations are recorded first, and then
// applied to the DaemonSet in a single controlled pass when Apply() is called.
//
...
// EditDaemonSetSpec records a mutation for the DaemonSet's top-level spec.
//
// Planning:
// All DaemonSet spec edits are stored and executed during Apply().
//
// Execution Order:
//   - Within a feature, edits are applied in registration order.
//   - Overall, DaemonSet spec edits are executed AFTER daemonset-metadata edits but BEFORE pod template/spec/container edits within the same feature.
//
// If the edit function is nil, the registration is ignored.
func (m *Mutator) EditDaemonSetSpec(edit func(*editors.DaemonSetSpecEditor) error) {
	if edit == nil {
		return
	}
	m.requireActivePlan()
...
// EditPodSpec records a mutation for the DaemonSet's pod spec.
//
// Planning:
...
// EditPodTemplateMetadata records a mutation for the DaemonSet's pod template metadata.
//
// Planning:
...
// Execution Order:
//   - Within a feature, edits are applied in registration order.
//   - Overall, pod template metadata edits are executed AFTER daemonset-metadata/spec edits but BEFORE pod spec/container edits within the same feature.
//
// If the edit function is nil, the registration is ignored.
...
// EditObjectMetadata records a mutation for the DaemonSet's own metadata.
//
// Planning:
...
// in all containers of the DaemonSet.
//
...
// removed from all containers of the DaemonSet.
//
// This is a convenience wrapper over EditContainers.
...
// Apply executes all recorded mutation intents on the underlying DaemonSet.
//
// Execution Order:
...
//   - Later features observe the DaemonSet as modified by all previous features.
//
// Timing:
// No changes are made to the DaemonSet until Apply() is called.
// Selectors and edit functions are executed during this pass.
func (m *Mutator) Apply() error {
	for _, plan := range m.plans {
		// 1. Object metadata
		if len(plan.daemonsetMetadataEdits) > 0 {
			editor := editors.NewObjectMetaEditor(&m.current.ObjectMeta)
			for _, edit := range plan.daemonsetMetadataEdits {
		...
		// 2. DaemonSetSpec
		if len(plan.daemonsetSpecEdits) > 0 {
			editor := editors.NewDaemonSetSpecEditor(&m.current.Spec)
			for _, edit := range plan.daemonsetSpecEdits {
				if err := edit(editor); err != nil {
					return err
				}
			}
