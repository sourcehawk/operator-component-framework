...
import (
	...
	"github.com/sourcehawk/operator-component-framework/examples/role-primitive/app"
	"github.com/sourcehawk/operator-component-framework/examples/role-primitive/resources"
...
func main() {
	...
	controller := &app.ExampleController{
		...
		NewRoleResource: resources.NewRoleResource,
	}

	// 4. Run reconciliation with multiple spec variations to demonstrate how
	//    feature-gated mutations compose RBAC rules.
	specs := []sharedapp.ExampleAppSpec{
		...
		{
			Version:       "1.2.4",
			EnableTracing: false, // Disable tracing (removes secret access)
			EnableMetrics: true,
		},
		{
			Version:       "1.2.4",
