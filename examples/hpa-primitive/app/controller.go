...
type ExampleController struct {
	...
	// NewHPAResource is a factory function to create the HPA resource.
	// This allows us to inject the resource construction logic.
	NewHPAResource func(*ExampleApp) (component.Resource, error)
}
...
func (r *ExampleController) Reconcile(ctx context.Context, owner *ExampleApp) error {
	// 1. Build the HPA resource for this owner.
	hpaResource, err := r.NewHPAResource(owner)
	if err != nil {
		return err
	}

	// 2. Build the component that manages the HPA.
	comp, err := component.NewComponentBuilder().
		WithName("example-app").
		WithConditionType("AppReady").
		WithResource(hpaResource, component.ResourceOptions{}).
