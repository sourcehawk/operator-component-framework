...
type ExampleController struct {
	...
	// NewServiceResource is a factory function to create the service resource.
	// This allows us to inject the resource construction logic.
	NewServiceResource func(*sharedapp.ExampleApp) (component.Resource, error)
}
...
func (r *ExampleController) Reconcile(ctx context.Context, owner *sharedapp.ExampleApp) error {
	// 1. Build the service resource for this owner.
	svcResource, err := r.NewServiceResource(owner)
	if err != nil {
		return err
	}

	// 2. Build the component that manages the service.
	comp, err := component.NewComponentBuilder().
		WithName("example-app").
		WithConditionType("AppReady").
		WithResource(svcResource, component.ResourceOptions{}).
