...
type ExampleController struct {
	...
	// NewClusterRoleResource is a factory function to create the clusterrole resource.
	// This allows us to inject the resource construction logic.
	NewClusterRoleResource func(*sharedapp.ExampleApp) (component.Resource, error)
}
...
func (r *ExampleController) Reconcile(ctx context.Context, owner *sharedapp.ExampleApp) error {
	// 1. Build the clusterrole resource for this owner.
	crResource, err := r.NewClusterRoleResource(owner)
	if err != nil {
		return err
	}

	// 2. Build the component that manages the clusterrole.
	comp, err := component.NewComponentBuilder().
		WithName("example-app").
		WithConditionType("AppReady").
		WithResource(crResource, component.ResourceOptions{}).
		Build()
	if err != nil {
		return err
	}
