...
type FieldApplicationFlavor flavors.FieldApplicationFlavor[*appsv1.DaemonSet]

// PreserveCurrentLabels ensures that any labels present on the current live
// DaemonSet but missing from the applied (desired) object are preserved.
// If a label exists in both, the applied value wins.
func PreserveCurrentLabels(applied, current, desired *appsv1.DaemonSet) error {
	return flavors.PreserveCurrentLabels[*appsv1.DaemonSet]()(applied, current, desired)
}

// PreserveCurrentAnnotations ensures that any annotations present on the current
// live DaemonSet but missing from the applied (desired) object are preserved.
// If an annotation exists in both, the applied value wins.
func PreserveCurrentAnnotations(applied, current, desired *appsv1.DaemonSet) error {
	return flavors.PreserveCurrentAnnotations[*appsv1.DaemonSet]()(applied, current, desired)
}

// PreserveCurrentPodTemplateLabels ensures that any labels present on the
// current live DaemonSet's pod template but missing from the applied
// (desired) object's pod template are preserved.
// If a label exists in both, the applied value wins.
//
// Note: pod template metadata changes may affect the rollout hash of the DaemonSet.
func PreserveCurrentPodTemplateLabels(applied, current, _ *appsv1.DaemonSet) error {
...
// on the current live DaemonSet's pod template but missing from the applied
// (desired) object's pod template are preserved.
// If an annotation exists in both, the applied value wins.
//
// Note: pod template metadata changes may affect the rollout hash of the DaemonSet.
func PreserveCurrentPodTemplateAnnotations(applied, current, _ *appsv1.DaemonSet) error {
