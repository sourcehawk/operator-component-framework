...
func DefaultFieldApplicator(current, desired *appsv1.DaemonSet) error {
...
// Resource is a high-level abstraction for managing a Kubernetes DaemonSet within a controller's
// reconciliation loop.
//
...
//   - component.Suspendable: for graceful deactivation via deletion.
...
type Resource struct {
	base *generic.WorkloadResource[*appsv1.DaemonSet, *Mutator]
}

// Identity returns a unique identifier for the DaemonSet in the format
// "apps/v1/DaemonSet/<namespace>/<name>".
//
// This identifier is used by the framework's internal tracking and recording
// mechanisms to distinguish this specific DaemonSet from other resources
// managed by the same component.
func (r *Resource) Identity() string {
	return r.base.Identity()
}

// Object returns a copy of the underlying Kubernetes DaemonSet object.
//
...
// Mutate transforms the current state of a Kubernetes DaemonSet into the desired state.
//
...
// DeleteOnSuspend determines whether the DaemonSet should be deleted from the
...
// DaemonSets have no replicas field and cannot be scaled to zero in-place.
func (r *Resource) DeleteOnSuspend() bool {
	return r.base.DeleteOnSuspend()
}

// Suspend triggers the deactivation of the DaemonSet.
//
...
// because the DaemonSet is deleted on suspension rather than mutated.
...
// Suspended because the DaemonSet is deleted on suspension.
...
// from the reconciled DaemonSet.
//
// This is called by the framework after a successful reconciliation of the
// resource. It allows the component to export details that might be needed
