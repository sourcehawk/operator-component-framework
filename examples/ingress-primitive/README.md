...
This example demonstrates the usage of the `ingress` primitive within the operator component framework.
It shows how to manage a Kubernetes Ingress as a component of a larger application, utilizing features like:
...
- **Field Flavors**: Preserving labels and annotations that might be managed by external tools (e.g., cert-manager, ingress controllers).
...
- `resources/`: Contains the central `NewIngressResource` factory that assembles all features using the `ingress.Builder`.
