...
func TestMutate_OrderingAndFlavors(t *testing.T) {
	...
	t.Run("flavors run in registration order", func(t *testing.T) {
		...
		var order []string
		flavor1 := func(_, _, _ *networkingv1.Ingress) error {
			order = append(order, "flavor1")
			return nil
		}
		flavor2 := func(_, _, _ *networkingv1.Ingress) error {
	...
	t.Run("flavor error is returned with context", func(t *testing.T) {
		...
		flavor := func(_, _, _ *networkingv1.Ingress) error {
			return flavorErr
		}
...
func TestDefaultFlavors(t *testing.T) {
	t.Run("PreserveCurrentLabels", func(t *testing.T) {
		applied := &networkingv1.Ingress{ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"keep": "applied", "overlap": "applied"}}}
		current := &networkingv1.Ingress{ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"extra": "current", "overlap": "current"}}}
	...
	t.Run("PreserveCurrentAnnotations", func(t *testing.T) {
		applied := &networkingv1.Ingress{ObjectMeta: metav1.ObjectMeta{Annotations: map[string]string{"keep": "applied"}}}
		current := &networkingv1.Ingress{ObjectMeta: metav1.ObjectMeta{Annotations: map[string]string{"extra": "current"}}}
	...
	t.Run("handles nil maps safely", func(t *testing.T) {
		applied := &networkingv1.Ingress{}
		current := &networkingv1.Ingress{ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"extra": "current"}}}
