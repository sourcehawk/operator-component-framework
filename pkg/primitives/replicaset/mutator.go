...
// Mutation defines a mutation that is applied to a replicaset Mutator
...
type featurePlan struct {
	replicaSetMetadataEdits  []func(*editors.ObjectMetaEditor) error
	replicaSetSpecEdits      []func(*editors.ReplicaSetSpecEditor) error
...
// applied to the ReplicaSet in a single controlled pass when Apply() is called.
//
...
//
// Planning:
// All replicaset spec edits are stored and executed during Apply().
//
// Execution Order:
...
func (m *Mutator) EditObjectMetadata(edit func(*editors.ObjectMetaEditor) error) {
	...
	m.active.replicaSetMetadataEdits = append(m.active.replicaSetMetadataEdits, edit)
}
...
// EnsureContainer records that a regular container must be present in the ReplicaSet.
...
// EnsureInitContainer records that an init container must be present in the ReplicaSet.
...
// in all containers of the ReplicaSet.
//
...
// removed from all containers of the ReplicaSet.
//
// This is a convenience wrapper over EditContainers.
...
func (m *Mutator) Apply() error {
	for _, plan := range m.plans {
		...
		// 2. ReplicaSetSpec
		if len(plan.replicaSetSpecEdits) > 0 {
			editor := editors.NewReplicaSetSpecEditor(&m.current.Spec)
			for _, edit := range plan.replicaSetSpecEdits {
				if err := edit(editor); err != nil {
					return err
				}
			}
